  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if ("errors" in dataRequest) {
        ...
        dataRequest = await getUserDataAsync(dataUserLocal.id, dataUpdate.accessToken);
      }
      ...
      for (let item of dataRequest.notification) {
        ...
        let tmpData = await getUserDataLiteAsync(item.content.userDtoId, dataUserLocal.accessToken);
    ...
    >
      <View style={{ flex: 1, marginRight: 5 }}>
        {data.detail.avatarUrl ? (
          <Image style={cpnNotiStyles.avt} source={{ uri: data.detail.avatarUrl }} />
        ) : (
          <Image style={cpnNotiStyles.avt} source={{uri: "https://firebasestorage.googleapis.com/v0/b/testgame-d8af2.appspot.com/o/avt.png?alt=media&token=b8108af6-1f90-4512-91f5-45091ca7351f"}} />
        )}
      ...
      > 
      {data.type === "NEWPOST" ? (
        <Text style={cpnNotiStyles.name}>{`${data.detail.name}`} vừa đăng một bài</Text>
      ) : data.type ==="NEWINTERACTION" ? (
        <Text style={cpnNotiStyles.name}>{data.detail.name} vừa thích bài đăng của bạn</Text>
      ) : data.type ==="NEWCOMMENT" && (
        <Text style={cpnNotiStyles.name}>{data.detail.name} vừa bình luận bài đăng của bạn</Text>
      )}
